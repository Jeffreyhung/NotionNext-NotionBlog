      ...
      >
        <div className="lg:p-8 p-4 flex flex-col w-full">
          ...
          <div className="text-gray-400 justify-between flex">
            <Link href={`/category/${post.category}`} className="cursor-pointer font-light text-sm hover:underline hover:text-indigo-700 dark:hover:text-indigo-400 transform">
        ...
        {CONFIG_HEXO.POST_LIST_COVER && !showPreview && post?.page_cover && !post.results && (
          <Link href={`${BLOG.SUB_PATH}/${post.slug}`} className="flex w-full relative duration-200 rounded-t-xl lg:rounded-r-xl lg:rounded-t-none cursor-pointer transform overflow-hidden">
