...
const HexoRecentComments = (props) => {
  ...
  return <Card >
        ...
        {!onLoading && comments && comments.length > 0 && comments.map((comment) => <div key={comment.objectId} className='pb-2 pl-1'>
            ...
            <div className='dark:text-gray-400 text-gray-400  text-sm text-right cursor-pointer hover:text-red-500 hover:underline pt-1 pr-2'><Link href={{ pathname: comment.url, hash: comment.objectId, query: { target: 'comment' } }} className="hover:text-red-500 hover:underline">-- {comment.nick}</Link></div>
        </div>)}
