...
const MenuButtonGroupTop = (props) => {
  ...
  return <nav id='nav' className='leading-8 flex justify-center  font-light w-full'>
    {links.map(link => {
      if (link.show) {
        return <Link key={`${link.to}`} title={link.to} href={link.to} target={link.to.indexOf('http') === 0 ? '_blank' : '_self'} className={'py-1.5 my-1 px-3  text-base justify-center items-center cursor-pointer'} >
